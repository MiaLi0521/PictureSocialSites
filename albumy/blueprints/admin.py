...
def index():
    ...
    return render_template('admin/index.html', user_count=user_count, photo_count=photo_count, tag_count=tag_count,
                           ...
                           blocked_user_count=blocked_user_count, reported_photos_count=reported_photos_count,
...
def manage_user():
    ...
    pagination = filtered_users.paginate(page=page, per_page=per_page)
...
def manage_photo(order):
    ...
    if order == 'by_time':
        ...
        pagination = Photo.query.order_by(Photo.flag.desc()).paginate(page=page, per_page=per_page)
...
def manage_comment(order):
    ...
    if order == 'by_time':
        ...
        pagination = Comment.query.order_by(Comment.flag.desc()).paginate(page=page, per_page=per_page)
...
def manage_tag():
    ...
    pagination = Tag.query.join(Tag.photos).group_by(Tag.id).order_by(func.count(Photo.id).desc()).\
