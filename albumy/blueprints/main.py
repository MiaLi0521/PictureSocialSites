...
def index():
    if current_user.is_authenticated:
        ...
        pagination = Photo.query.join(Follow, Photo.author_id == Follow.followed_id).\
            filter_by(follower_id=current_user.id).order_by(Follow.timestamp.desc()).\
...
def explore():
    # order_by()方法会扫描整张表，当数据量增多时，random()函数的性能也会逐渐下降，可以考虑通过扫描索引（主键）来优化
    # 随机函数是不同数据库引擎特定的函数，random()只适用于PostgreSQL和SQLite；
...
def show_collectors(photo_id):
    ...
    pagination = Collect.query.with_parent(photo).order_by(Collect.timestamp.asc()).paginate(page=page, per_page=per_page)
    collects = pagination.items
    return render_template('main/collectors.html', collects=collects, photo=photo, pagination=pagination)


@main_bp.route('/photo/<photo_id>/description', methods=['POST'])
@login_required
...
def new_comment(photo_id):
    ...
    if form.validate_on_submit():
        ...
        if replied_id and not send_flag and current_user.receive_comment_notification:
        ...
        db.session.commit()
        if current_user != photo.author and not send_flag and current_user.receive_comment_notification:
...
@main_bp.route('/delete/photo/<photo_id>', methods=['POST'])
@login_required
...
@main_bp.route('/delete/comment/<comment_id>', methods=['POST'])
@login_required
...
def show_notifications():
    ...
    if filter_rule == 'unread':
        notifications.filter_by(is_read=False)
...
def read_all_notification():
    notifications = Notification.query.with_parent(current_user).filter_by(is_read=False)
